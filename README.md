...
resource "aws_lambda_function" "example" {
  ...
  layers = [
    module.bref_layers.runtime_layer_arn
  ]
}
```
...
Since Bref v3, the runtime has been consolidated into a single layer, which means you can use the same runtime for all deployment types.

Your configuration can be updated to use the consolidated runtime layer as shown below:
...
resource "aws_lambda_function" "worker" {
  # ... other configuration
  # Was: layers = [module.bref_layers.function_layer_arn]
  layers = [module.bref_layers.runtime_layer_arn]
}

# FPM runtime for HTTP applications
resource "aws_lambda_function" "web" {
  # ... other configuration
  # Was: layers = [module.bref_layers.fpm_layer_arn]
  layers = [module.bref_layers.runtime_layer_arn]
}
...
resource "aws_lambda_function" "console" {
  # ... other configuration
  # Was: layers = [module.bref_layers.console_layer_arn]
  layers = [module.bref_layers.runtime_layer_arn]
}
```
...
resource "aws_lambda_function" "app_with_extensions" {
  ...
  layers = module.bref_layers.runtime_layers
}
```
...
| bref_catalog_url            | Optional override for the Bref runtime catalog URL. Defaults to v3 master branch    | `string`       | `null`  |    no    |
| bref_extensions_catalog_url | Optional override for the Bref extensions catalog URL. Defaults to v3 master branch | `string`       | `null`  |    no    |
...
| Name                 | Description                                                                 |
|----------------------|-----------------------------------------------------------------------------|
| runtime_layer_arn    | ARN of the Bref PHP runtime layer                                           |
| extension_layer_arns | Map of PHP extension names to their layer ARNs                              |
| runtime_layers       | Array containing the runtime layer ARN and all extension layer ARNs         |
...
Override these URLs using `bref_catalog_url` or `bref_extensions_catalog_url` to pin v3 to specific versions.
